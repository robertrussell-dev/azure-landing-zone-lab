...
The rule has to be written down, because the intuitive question, "is it
internet facing?", doesn't determine the archetype.
...
2. Is it experimentation with no promotion path to production? Place under
   **Sandboxes**. Isolated from the hub, with loose policy.
...
Internet exposure isn't in that list. It's answered inside the landing zone,
not by choosing one.
...
Row two is the important one. A public customer portal
...
Note the word monolithic. The next section covers the more common, and usually
better, answer.
...
If an application splits into components with different connectivity needs,
the answer is usually two landing zones. The public front end goes to Online
and the component that talks to on premises goes to Corp. Same rule, applied
per component.

That is normally the better design, because it minimizes what needs corporate
...
The cost: two landing zones means two
...
so egress can be inspected centrally. Here that's a Deny on public IPs attached
to network interfaces, assigned at Corp and not at Online, where direct
internet access is expected.

## The cost of getting it wrong

Moving a subscription between archetype management groups has consequences, in
...
**Addressing can't be retrofitted.** Corp requires
...
That's why the connectivity question is asked at intake: addressing is the one
part of this decision a management group move can't undo.

## Where this rule breaks

Two cases the rule handles badly.
...
Network security groups and firewall rules handle that case, outside the
archetype. Where direction matters, the workload still needs a network design
conversation.

## Alternatives considered

**Place by business unit or by application team.** Rejected. Microsoft's
guidance is explicit that the hierarchy should not re-create the organizational
chart, and organizational structure changes more often than connectivity
requirements do. A reorganization should not trigger a subscription migration.
...
**Ask the workload team to choose.** Rejected. They optimize for their own
...
- Sandboxes have no promotion path. A workload that turns out to matter gets a
  new landing zone, because the sandbox ran under loose policy.
